                ...
                <div class="card-header bg-primary text-white">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="mb-0">
                            ...
                            Units Management - {{ $brand ? $brand->brand_name : 'No Brand Found' }}
                        </h4>
                        <div>
                            ...
                            <a href="{{ route('categories.SuperAdminIndex') }}" class="btn btn-light btn-sm ms-2">
                                <i class="bi bi-tags me-1"></i> Categories
                            </a>
                            <a href="{{ route('brands.SuperAdminIndex', $categoryID) }}" class="btn btn-light btn-sm ms-2">
                                ...
                                <tbody>
                                    @foreach($units as $index => $unit)
                                        <tr>
                                            ...
                                            <td>{{ \Carbon\Carbon::parse($unit->created_at)->format('M d, Y') }}</td>
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center gap-2">
                                                    ...
                                                    <!-- Delete Button -->
                                                    <form action="{{ route('units.SuperAdmindestroy', ['id' => $unit->id, 'brandID' => $brandID, 'categoryID' => $categoryID]) }}" 
                                                          method="POST" 
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body p-4">
                <form id="createUnitForm" action="{{ route('units.SuperAdminstore', ['brandID' => $brandID, 'categoryID' => $categoryID]) }}" method="POST">
                    @csrf
                    <div class="mb-4">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.edit-unit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const formAction = `/superAdmin/units/${unitId}/update/brand/{{ $brandID }}/category/{{ $categoryID }}`;
        ...
        async function checkUnitExists(unitName) {
            try {
                const response = await fetch("{{ route('units.SuperAdmincheck') }}", {
                    method: 'POST',
                    headers: {
